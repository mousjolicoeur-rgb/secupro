...
// ✅ planning_detecte ajouté
interface AIImportResult {
  type_detecte?:       string;
  nb_lignes?:          number;
  colonnes_detectees?: string[];
  agents_detectes?:    string[];
  planning_detecte?:   { nom: string; site: string; horaires: string; statut: string }[];
  resume?:             string;
  avertissements?:     string[];
  action_suggeree?:    string;
}

type PlanningLine = { nom: string; site: string; horaires: string; statut: string };
...
// ✅ callback reçoit le planning complet
function useImport(onPlanningImported: (planning: PlanningLine[]) => void) {
  ...
  const analyzeWithAI = async () => {
    ...
    try {
      // ✅ Prompt mis à jour pour extraire planning_detecte avec sites et horaires
...
Analyse ce fichier et retourne UNIQUEMENT un JSON (sans balises markdown) avec :
{
  ...
  "planning_detecte": [
    { "nom": "Prenom NOM", "site": "nom du site", "horaires": "HH:MM-HH:MM", "statut": "actif|disponible|alerte|critique" }
  ],
  "resume": "1 phrase décrivant le contenu",
  "avertissements": ["..."],
  "action_suggeree": "description de l'import"
}
IMPORTANT : si les colonnes site et horaires sont présentes dans le fichier, remplis absolument planning_detecte pour CHAQUE ligne avec les vraies valeurs extraites. Pour le champ nom, concatene prenom + nom en majuscules. Ne laisse pas de champs vides.`;
  ...
  // ✅ confirmImport passe le planning complet (avec sites + horaires)
  const confirmImport = (onDone: () => void) => {
    if (aiResult?.planning_detecte?.length) {
      onPlanningImported(aiResult.planning_detecte);
    } else if (aiResult?.agents_detectes?.length) {
      // fallback si pas de planning_detecte
      onPlanningImported(
        aiResult.agents_detectes.map((n) => ({ nom: n, site: "—", horaires: "—", statut: "disponible" }))
      );
    }
    setImportDone(true);
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "24px", flexShrink: 0 }}>
        ...
        <nav style={{ display: "flex", alignItems: "stretch", height: "56px", gap: "2px" }}>
          {([
            ...
            active ? (
              <div key={label} style={{ display: "flex", alignItems: "center", padding: "0 14px", fontSize: "13px", fontWeight: 600, color: L.blue, borderBottom: `2px solid ${L.blue}`, cursor: "default" }}>{label}</div>
            ) : (
              <Link key={label} href={href} style={{ display: "flex", alignItems: "center", padding: "0 14px", fontSize: "13px", fontWeight: 400, color: L.textSec, borderBottom: "2px solid transparent", textDecoration: "none", cursor: "pointer" }}>{label}</Link>
            )
          )}
        </nav>
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
        <span style={{ fontSize: "12px", color: L.textMuted, fontVariantNumeric: "tabular-nums", marginRight: "4px" }}>{time}</span>
        <span style={{ ...pill("#F0FDF4", L.green), border: "1px solid #BBF7D0", padding: "3px 10px" }}>
        ...
        <button type="button" style={btnSecondary} onClick={onExit}><ArrowLeft size={14} /> Retour</button>
        <button type="button" style={btnSecondary} onClick={onReset}><RefreshCw size={14} /> Réinitialiser</button>
        <button type="button" style={btnPrimary}   onClick={onImport}><Upload size={14} /> Importer</button>
        <button type="button" style={btnSecondary} onClick={onSignOut}><LogOut size={14} /> Déconnexion</button>
      </div>
    </header>
  );
}
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "12px" }}>
      {items.map(({ icon: Icon, val, label, sub, iconBg, iconColor }) => (
        <div key={label} style={{ ...card, padding: "20px", display: "flex", alignItems: "center", gap: "16px" }}>
          <div style={{ width: 40, height: 40, borderRadius: "8px", flexShrink: 0, background: iconBg, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ overflowX: "auto" }}>
        {agents.length === 0 ? (
          <div style={{ padding: "24px 16px", textAlign: "center", fontSize: "13px", color: L.textMuted }}>
            Aucun planning — utilisez Importer pour charger un fichier
          </div>
        ) : (
            ...
            <thead>
              <tr style={{ background: L.bgMuted, borderBottom: `2px solid ${L.border}` }}>
                {["Agent", "Site", "Horaires", "Statut"].map((h, i) => (
                  <th key={h} style={{ ...sectionLabel, padding: "10px 16px", textAlign: i === 3 ? "center" : "left", fontWeight: 600 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {agents.map((a, i) => {
                ...
                return (
                  <tr key={a.id} style={{ borderBottom: i < agents.length - 1 ? `1px solid ${L.borderSub}` : "none" }}
...
function SitesActifs({ sites }: { sites: Site[] }) {
  return (
    <Panel title="Sites actifs" icon={MapPin} badge={<span style={pill(L.blueBg, L.blueText)}>{sites.length} sites</span>}>
      {sites.length === 0 ? (
        <div style={{ padding: "24px 16px", textAlign: "center", fontSize: "13px", color: L.textMuted }}>Aucun site actif</div>
      ) : sites.map((s, i) => {
        const cfg = SITE_CFG[s.statut];
        return (
          <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 16px", borderBottom: i < sites.length - 1 ? `1px solid ${L.borderSub}` : "none" }}>
            <div>
...
function AnomaliesPointage({ anomalies }: { anomalies: Anomalie[] }) {
  return (
    <Panel title="Anomalies pointage" icon={AlertTriangle} badge={<span style={pill(L.redBg, L.redText)}>{anomalies.length} alerte{anomalies.length > 1 ? "s" : ""}</span>}>
      <div style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: "8px" }}>
        {anomalies.length === 0 ? (
          <p style={{ fontSize: "13px", color: L.textMuted }}>Aucune anomalie</p>
        ) : anomalies.map((a, i) => (
          <div key={i} style={{ background: L.bgMuted, borderRadius: "6px", border: `1px solid ${L.border}`, padding: "12px 14px" }}>
...
function AlertesIncidents({ alertes }: { alertes: Alerte[] }) {
  ...
  return (
    <Panel title="Alertes & incidents" icon={Bell} badge={<span style={pill(L.redBg, L.redText)}>{critiques} critique{critiques > 1 ? "s" : ""}</span>}>
      {alertes.length === 0 ? (
        <div style={{ padding: "24px 16px", textAlign: "center", fontSize: "13px", color: L.textMuted }}>Aucune alerte</div>
      ) : (
        <div style={{ maxHeight: "240px", overflowY: "auto" }}>
          {alertes.map((a, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "10px", padding: "10px 16px", borderBottom: i < alertes.length - 1 ? `1px solid ${L.borderSub}` : "none" }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: "8px", flex: 1 }}>
                <span style={{ width: 7, height: 7, borderRadius: "50%", flexShrink: 0, marginTop: "5px", background: ALERTE_DOT[a.type] }} />
    ...
    <Panel title="IA Business" icon={Bot} badge={<span style={pill(L.greenBg, L.greenText)}>Actif</span>}>
      <div style={{ padding: "12px 16px" }}>
        {remplacements.length === 0 ? (
          <div style={{ fontSize: "13px", color: L.textMuted, textAlign: "center", padding: "12px 0" }}>Aucune absence détectée</div>
        ) : (
          <>
            <div style={{ padding: "10px 12px", background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: "6px", fontSize: "12px", color: "#1E40AF", lineHeight: 1.5, marginBottom: "14px" }}>
...
function GestionAgents() {
  ...
  useEffect(() => {
    ...
    async function fetchCounts() {
      try {
        ...
        const { data } = await supabase.from("agent_societe").select("status").eq("societe_id", user.id);
  ...
  return (
    <Panel title="Gestion des agents" icon={Users} badge={<span style={pill(L.blueBg, L.blueText)}>{loading ? "…" : `${approuves + enAttente} liés`}</span>}>
    ...
    <Panel title="Infractions CNAPS les plus fréquentes" icon={ShieldAlert}>
      <div style={{ padding: "14px 16px", borderBottom: `1px solid ${L.borderSub}` }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
          <span style={{ fontSize: "13px", fontWeight: 600, color: L.text }}>{score}/10 points de contrôle validés</span>
          {riskAlert && <span style={{ ...pill(L.redBg, L.redText), fontWeight: 700 }}>⚠ RISQUE DE CONTRÔLE CNAPS</span>}
        </div>
        <div style={{ height: "4px", background: L.border, borderRadius: "999px", overflow: "hidden" }}>
          <div style={{ height: "100%", borderRadius: "999px", width: `${(score / 10) * 100}%`, background: barColor, transition: "width 0.3s ease, background 0.3s ease" }} />
        </div>
      </div>
      {INFRACTIONS.map((inf, i) => (
        <button key={inf.num} type="button" onClick={() => toggle(i)} style={{ display: "flex", alignItems: "center", gap: "12px", width: "100%", height: "48px", padding: "0 16px", background: "transparent", border: "none", borderBottom: i < INFRACTIONS.length - 1 ? `1px solid ${L.borderSub}` : "none", cursor: "pointer", textAlign: "left" }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = L.bgMuted; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "transparent"; }}
        >
          <span style={{ width: "22px", height: "20px", borderRadius: "4px", flexShrink: 0, background: checked[i] ? L.greenBg : L.bgMuted, color: checked[i] ? L.greenText : L.textMuted, fontSize: "10px", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}>{inf.num}</span>
          <span style={{ width: "16px", height: "16px", borderRadius: "4px", flexShrink: 0, border: `1.5px solid ${checked[i] ? L.blue : "#CBD5E1"}`, background: checked[i] ? L.blue : L.bgCard, display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}>
            {checked[i] && <svg width="10" height="8" viewBox="0 0 10 8" fill="none"><path d="M1 4L3.5 6.5L9 1" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>}
          </span>
          <span style={{ flex: 1, fontSize: "13px", color: checked[i] ? L.textMuted : L.text, textDecoration: checked[i] ? "line-through" : "none", textDecorationColor: L.textMuted, transition: "color 0.15s" }}>{inf.label}</span>
          {inf.badge && <span style={{ ...pill(L.redBg, L.redText), fontSize: "10px", flexShrink: 0 }}>{inf.badge}</span>}
          <span style={{ ...pill(checked[i] ? L.greenBg : L.amberBg, checked[i] ? L.greenText : L.amberText), flexShrink: 0, transition: "all 0.15s" }}>{checked[i] ? "Conforme" : "À vérifier"}</span>
        </button>
      ))}
    </Panel>
  );
...
function ModalImport({ imp, onClose }: { imp: ImportHook; onClose: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={{ background: L.bgCard, border: `1px solid ${L.border}`, borderRadius: "12px", width: "520px", maxWidth: "95vw", padding: "24px", boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
          ...
          <button type="button" onClick={onClose} style={{ background: "transparent", border: "none", color: L.textMuted, cursor: "pointer", padding: "4px" }}><X size={18} /></button>
        </div>

        <div style={{ ...sectionLabel, marginBottom: "8px" }}>Format du fichier</div>
        <div style={{ display: "flex", gap: "8px", marginBottom: "6px" }}>
          {["CSV", "Excel", "PDF"].map((f) => (
            <button key={f} type="button" onClick={() => setFileType(f)} style={{ flex: 1, height: "36px", borderRadius: "6px", cursor: "pointer", background: fileType === f ? L.blueBg : L.bgMuted, border: `1px solid ${fileType === f ? L.blue : L.border}`, color: fileType === f ? L.blue : L.textSec, fontSize: "13px", fontWeight: fileType === f ? 600 : 400 }}>{f}</button>
          ))}
        </div>
        ...
        <div onClick={() => fileRef.current?.click()}
          ...
          style={{ border: `2px dashed ${dragOver ? L.blue : L.border}`, borderRadius: "8px", padding: "28px", textAlign: "center", cursor: "pointer", background: dragOver ? L.blueBg : L.bgMuted, transition: "all 0.15s" }}
        >
          {fileName ? (
            <>
            ...
            <>
              ...
              <div style={{ fontSize: "13px", color: L.textSec }}>Glissez votre fichier ici ou <span style={{ color: L.blue, fontWeight: 500 }}>cliquez pour parcourir</span></div>
              <div style={{ fontSize: "12px", color: L.textMuted, marginTop: "4px" }}>{FILE_ACCEPTS[fileType]}</div>
            </>
          )}
        </div>
        <input ref={fileRef} type="file" accept={FILE_ACCEPTS[fileType]} style={{ display: "none" }} onChange={(e) => handleFile(e.target.files?.[0])} />
        ...
        {aiResult && !importDone && (
          <div style={{ background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: "8px", padding: "14px", marginTop: "12px", fontSize: "12px", lineHeight: 1.6, maxHeight: "200px", overflowY: "auto" }}>
            ...
            {/* ✅ Confirmation que le planning avec sites + horaires a été détecté */}
            {!!aiResult.planning_detecte?.length   && (
              <div style={{ marginBottom: "4px", color: L.green, fontWeight: 600 }}>
                ✓ {aiResult.planning_detecte.length} lignes planning détectées (sites + horaires inclus)
              </div>
            )}
        ...
        {importDone && (
          <div style={{ textAlign: "center", padding: "14px", fontSize: "13px", fontWeight: 600, color: L.green, background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: "8px", marginTop: "12px", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
            ✓ Import confirmé — sites et horaires mis à jour
          </div>
        )}
...
function ModalConfirm({ icon, accentColor, title, message, labelConfirm, onConfirm, onCancel }: ModalConfirmProps) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }} onClick={(e) => e.target === e.currentTarget && onCancel()}>
      <div style={{ background: L.bgCard, border: `1px solid ${L.border}`, borderRadius: "10px", width: "380px", padding: "28px", textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.12)" }}>
        ...
        <div style={{ display: "flex", gap: "10px", justifyContent: "center" }}>
          ...
          <button type="button" onClick={onConfirm} style={accentColor === L.red ? btnDestructive : { ...btnPrimary, background: accentColor }}>
            {labelConfirm}
          </button>
        </div>
      </div>
...
export default function ChefExploitationDashboard() {
  ...
  // ✅ callback reçoit le planning complet → met à jour agents ET sites
  const imp = useImport((planning: PlanningLine[]) => {
    // Mise à jour agents avec site + horaires
    setAgents((prev) => {
      // Mettre à jour les agents existants
      const updated = prev.map((a) => {
        const match = planning.find((p) => p.nom === a.nom);
        if (!match) return a;
        return {
          ...a,
          site:     match.site     || a.site,
          horaires: match.horaires || a.horaires,
          statut:   (["actif", "alerte", "critique", "disponible"].includes(match.statut) ? match.statut : a.statut) as Statut,
        };
      });
      // Ajouter les nouveaux agents
      const existing = new Set(prev.map((a) => a.nom));
      const toAdd = planning
        .filter((p) => !existing.has(p.nom))
        .map((p, i): Agent => ({
          id: prev.length + i + 1,
          nom:      p.nom,
          site:     p.site     || "—",
          horaires: p.horaires || "—",
          statut:   (["actif", "alerte", "critique", "disponible"].includes(p.statut) ? p.statut : "disponible") as Statut,
        }));
      return [...updated, ...toAdd];
    });

    // ✅ Reconstruire les sites actifs depuis le planning importé
    const sitesUniques = [...new Set(
      planning.map((p) => p.site).filter((s) => s && s !== "—")
    )];
    if (sitesUniques.length > 0) {
      setSites(sitesUniques.map((nom) => {
        const agentsDuSite = planning.filter((p) => p.site === nom);
        const hasCritique  = agentsDuSite.some((p) => p.statut === "critique");
        const hasAlerte    = agentsDuSite.some((p) => p.statut === "alerte");
        return {
          nom,
          ville:    "Lyon",
          effectif: `${agentsDuSite.length}/${agentsDuSite.length}`,
          statut:   hasCritique ? "CRITIQUE" : hasAlerte ? "ALERTE" : "OK",
        } as Site;
      }));
    }
  });
  ...
  return (
    <div className={dmSans.className} style={{ background: L.bgPage, minHeight: "100vh", color: L.text, fontSize: "14px" }}>
      ...
      {toast && (
        <div style={{ position: "fixed", bottom: "24px", left: "50%", transform: "translateX(-50%)", zIndex: 10000, pointerEvents: "none", display: "flex", alignItems: "center", gap: "8px", padding: "12px 20px", borderRadius: "8px", background: L.bgCard, border: `1px solid ${L.border}`, boxShadow: "0 8px 24px rgba(0,0,0,0.1)", fontSize: "13px", fontWeight: 600, color: L.green }}>
