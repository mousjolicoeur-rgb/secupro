...
type FocusField = "email" | "password" | "confirm";

export default function RegisterSocietePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Marquer le rôle société dans la table profiles
      if (data.user) {
        await supabase.from("profiles").upsert({ id: data.user.id, role: "societe" });
      }
      ...
      <div className="relative w-full max-w-[440px] rounded-2xl px-8 py-9" style={{
        ...
        {emailSent ? (
          <div className="flex flex-col items-center text-center py-4 gap-4">
            ...
            <p className="text-[13px] leading-relaxed max-w-[320px]" style={{ color: LABEL_CLR }}>
              Un lien d&apos;activation a été envoyé à{" "}
              <strong style={{ color: CYAN }}>{email}</strong>.
              <br />Cliquez dessus pour activer votre espace société.
            </p>
            <Link href="/login" style={{
              display: "inline-block", marginTop: 8,
              color: CYAN, fontWeight: 700, textDecoration: "none", fontSize: 13,
            }}>
          ...
          <>
            ...
            <div className="flex flex-col items-center gap-3 mb-6">
              ...
              <div className="text-center">
                ...
                <h1 className="text-[1.4rem] font-black leading-tight">Créer un compte Société</h1>
                <p className="text-[11px] font-medium mt-1" style={{ color: "rgba(0,209,255,0.45)" }}>
                  Essai gratuit 1 mois · Aucune carte requise
                </p>
              </div>
            </div>
            ...
            <form className="flex flex-col gap-4" onSubmit={handleSubmit} noValidate>
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="email"
                  ...
                  <AtSign size={10} style={{ color: CYAN }} /> Email professionnel de votre société *
                </label>
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="password"
                  ...
                  <span style={{ color: "rgba(0,209,255,0.3)", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>(min. 8 car.)</span>
                </label>
                <div className="flex items-center rounded-xl transition-all duration-200" style={{
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center rounded-xl transition-all duration-200" style={{
                  ...
                  <input
                    id="confirm" type={showCfm ? "text" : "password"} value={confirm} required disabled={loading}
                    onChange={e => setConfirm(e.target.value)}
                    onFocus={() => setFocused("confirm")} onBlur={() => setFocused(null)}
                    placeholder="••••••••••••"
                    autoComplete="new-password"
                    className="flex-1 bg-transparent px-3 py-3 text-[13px] font-semibold outline-none placeholder:text-[rgba(148,163,184,0.22)] placeholder:font-medium"
                    style={{ color: "#f1f5f9", fontFamily: "var(--font-geist-mono),'Courier New',monospace", letterSpacing: showCfm ? "0.12em" : "0.22em" }}
                  />
                  <button type="button" onClick={() => setShowCfm(v => !v)} aria-label={showCfm ? "Masquer" : "Afficher"}
                    className="mr-3 p-1 rounded-md"
                    style={{ color: showCfm ? CYAN : "rgba(0,209,255,0.3)", background: "none", border: "none", cursor: "pointer" }}>
            ...
            <p className="mt-5 text-center text-[12px]" style={{ color: "rgba(148,163,184,0.5)" }}>
              Déjà un compte ?{" "}
              <Link href="/login" style={{ color: CYAN, fontWeight: 700, textDecoration: "none" }}>
                Se connecter →
              </Link>
            </p>
            <p className="mt-3 text-center text-[11px]" style={{ color: TEXT_MUT }}>
              Vous êtes agent ?{" "}
              <Link href="/agent/register" style={{ color: "rgba(0,209,255,0.5)", fontWeight: 600, textDecoration: "none" }}>
