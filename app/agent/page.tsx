...
import { useRouter } from 'next/navigation'; // 🚀 L'arme secrète de Next.js
...
export default function AgentPage() {
  ...
  const handleActivation = async () => {
    ...
    // On vérifie le code dans ta table Supabase "entreprises"
    const entreprise = await checkActivationCode(code.toUpperCase());

    if (entreprise) {
      // SUCCESS : On stocke les infos pour l'app
      localStorage.setItem('entreprise_id', entreprise.id);
      localStorage.setItem('entreprise_nom', entreprise.nom);
      
      // 🚀 On utilise le routeur officiel pour changer de page (Next.js adore ça)
    ...
    <div className="min-h-screen bg-[#0A1F2F] flex flex-col items-center justify-center p-6 font-sans">
      <div className="w-full max-w-sm">
        <div className="text-center mb-10">
          <h1 className="text-[#00D1FF] text-4xl font-black tracking-tighter mb-2">SECUPRO <span className="text-white">PRO</span></h1>
          <p className="text-gray-500 text-xs uppercase tracking-[0.3em]">Activation de mission</p>
        </div>
