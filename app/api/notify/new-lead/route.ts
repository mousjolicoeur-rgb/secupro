...
export async function POST(req: NextRequest) {
  ...
  const { prenom, nom, email, telephone } = await req.json() as {
  ...
  try {
      ...
      html: `
        <div style="font-family:sans-serif;background:#050a12;color:#fff;padding:32px;border-radius:12px;max-width:480px">
          ...
          <table style="width:100%;border-collapse:collapse">
            <tr><td style="padding:8px 0;color:#64748b;font-size:12px;text-transform:uppercase;letter-spacing:0.1em">Prénom</td>
    ...
    if (email) {
      await resend.emails.send({
        ...
        html: `
          <div style="font-family:sans-serif;background:#050a12;color:#fff;padding:32px;border-radius:12px;max-width:480px">
            <h1 style="color:#00d1ff;letter-spacing:0.1em;text-transform:uppercase;font-size:22px;margin:0 0 8px">
              SECUPRO
            </h1>
            <p style="color:#64748b;font-size:11px;text-transform:uppercase;letter-spacing:0.25em;margin:0 0 28px">
              Système de gestion agents
            </p>
            <h2 style="font-size:16px;margin:0 0 16px">
              Bienvenue, ${prenom ?? "Agent"} !
            </h2>
            <p style="color:#94a3b8;line-height:1.7;font-size:14px">
              ...
              Votre dossier est désormais actif et en cours de traitement par notre équipe.
            </p>
            <div style="margin:28px 0;padding:16px;background:#0a1520;border:1px solid rgba(0,209,255,0.15);border-radius:8px">
              <p style="color:#00d1ff;font-size:12px;text-transform:uppercase;letter-spacing:0.2em;margin:0 0 8px;font-weight:700">
                Prochaines étapes
              </p>
            ...
            <p style="color:#475569;font-size:11px;margin:24px 0 0">
              © 2026 SECUPRO COMMAND SYSTEM · Cet email a été envoyé automatiquement, merci de ne pas y répondre.
            </p>
          </div>
        `,
      });
    ...
    console.error("[notify/new-lead]", err);
    // Non-blocking — ne pas faire échouer le save profil
